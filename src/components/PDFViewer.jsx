...
const PrintLayout = ({ header, children }) => {
  return (
...
export default function PDFViewer({ 
    ...
    if (index !== -1) {
      ...
      return (
        <>
          ...
          <span className="a4-clinic-subtitle" style={{ fontSize: subSize, color: 'var(--dark-light)', marginTop: '2px', fontWeight: 500, whiteSpace: 'nowrap' }}>{sub}</span>
        </>
      );
    }
    return (
      <>
        ...
        <span className="a4-clinic-subtitle" style={{ fontSize: subSize, color: 'var(--dark-light)', marginTop: '2px', fontWeight: 500, whiteSpace: 'nowrap' }}>คลินิกการประกอบโรคศิลปะสาขากิจกรรมบำบัด</span>
      </>
    );
  };
      ...
      <div className="a4-document repeating-header-doc" ref={documentRef} id="printable-a4-area" style={{ padding: '10mm 15mm 15mm 15mm', minHeight: '297mm' }}>
        ...
        >
          ...
          <div className="a4-patient-section">
            ...
            <div className="a4-data-item"></div>
          </div>
          ...
          {/* ส่วนที่ 2: Sensory Test */}
          {item.hasSensory !== false && (
            <>
              <h3 className="a4-table-title">ส่วนที่ {sectionCounter++}: สรุปผลการประเมินระบบประสาทความรู้สึก (Sensory Profile Test)</h3>
              <table className="a4-table">
                <thead>
                  <tr>
                    <th>ระบบการรับความรู้สึกที่ตรวจวัด (Sensory Systems)</th>
                    <th style={{ textAlign: 'center', width: '30%' }}>คะแนนที่ได้</th>
                  </tr>
                </thead>
                <tbody>
                  <tr><td>ระบบการรับภาพและการมองเห็น (Visual Processing)</td><td style={{ textAlign: 'center' }}>{item.sensoryScores?.visual ?? 0}</td></tr>
                  <tr><td>ระบบการได้ยินและการรับเสียง (Auditory Processing)</td><td style={{ textAlign: 'center' }}>{item.sensoryScores?.auditory ?? 0}</td></tr>
                  <tr><td>ระบบการเคลื่อนไหวและการควบคุมแกนกลาง (Movement Skills)</td><td style={{ textAlign: 'center' }}>{item.sensoryScores?.movement ?? 0}</td></tr>
                  <tr><td>ระบบการทรงตัวและระดับความตื่นตัว (Vestibular System)</td><td style={{ textAlign: 'center' }}>{item.sensoryScores?.vestibular ?? 0}</td></tr>
                  <tr><td>ระบบการรับรู้เอ็นข้อต่อและตำแหน่งในอวกาศ (Proprioceptive System)</td><td style={{ textAlign: 'center' }}>{item.sensoryScores?.proprioceptive ?? 0}</td></tr>
                  <tr><td>ระบบการรับสัมผัสทางผิวหนัง (Tactile System)</td><td style={{ textAlign: 'center' }}>{item.sensoryScores?.tactile ?? 0}</td></tr>
                  {isSensoryEnabled && (
          ...
          {item.hasSnap !== false && (
            <>
              <h3 className="a4-table-title">ส่วนที่ {sectionCounter++}: สรุปเกณฑ์การคัดกรองพฤติกรรมเสี่ยง (SNAP-IV Rating Scale)</h3>
              <table className="a4-table">
                <thead>
                  <tr>
        ...
        {pages.map((page, index) => (
          ...
          >
            <div style={{ flexShrink: 0 }}>
              {/* Header (แสดงหัวกระดาษเต็มทุกหน้า ตามสเปก) */}
                      ...
                      {row.isEmpty ? (
                        ...
                        <>
                          ...
                          <td style={{ padding: '7px 4px', verticalAlign: 'top', textAlign: 'center', fontWeight: 500 }}>
                            {formatTherapistName(row.therapist)}
                          </td>
                        </>
                      )}
                    </tr>
  ...
  const renderAnnualHolidays = () => {
    ...
    return (
      <div className="a4-document repeating-header-doc" ref={documentRef} id="printable-a4-area" style={{
        width: '210mm',
        minHeight: '297mm',
